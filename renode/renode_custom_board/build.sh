...
make
arm-none-eabi-objcopy -O binary renode_custom_board.elf renode_custom_board.bin
cd -

if ! [ -f flash.sh ]; then
    echo "st-flash --connect-under-reset write build/renode_custom_board.bin 0x08000000" >> flash.sh
    chmod +x flash.sh
fi
